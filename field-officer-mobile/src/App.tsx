...
import { assignmentValues, assignmentsForSection, displayStatus, formatCurrentLocation, getFormSections, isFormComplete, isReportLocked } from "./domain";
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.metricsGrid}>
        <KpiCard label="Assigned" value={assigned.length} note="Ready to start" icon="grid-outline" tone="green" onPress={() => onNavigate("Assignments")} />
        <KpiCard label="Due" value={dueThisWeek.length} note="Next visit" icon="time-outline" tone="amber" onPress={() => onNavigate("Assignments")} />
        <KpiCard label="Drafts" value={drafts.length} note="Not yet submitted" icon="document-text-outline" tone="blue" onPress={() => onNavigate("Drafts")} />
        <KpiCard label="To Sync" value={toSync.length} note="Ready to upload" icon="cloud-upload-outline" tone="violet" onPress={() => onNavigate("Sync")} />
      </View>
      {next ? (
...
function AssignmentList({ mode, onOpen }: { mode: "assignments" | "inspections" | "drafts"; onOpen: (item: Assignment) => void }) {
  ...
  const summaries = mode === "assignments"
    ? [
        { label: "Assigned", value: base.length, note: "Ready to start", icon: "folder-open-outline" as const, tone: "green" as const },
        { label: "Due this week", value: base.filter((item) => new Date(item.dueDate).getTime() <= Date.now() + 7 * 86_400_000).length, note: "Next 7 days", icon: "calendar-outline" as const, tone: "amber" as const },
        { label: "States", value: new Set(base.map((item) => item.state)).size, note: "Coverage area", icon: "map-outline" as const, tone: "blue" as const },
      ]
    : mode === "drafts"
      ? [
          { label: "Autosaved", value: base.length, note: "Saved automatically", icon: "cloud-done-outline" as const, tone: "green" as const },
          { label: "With evidence", value: base.filter((item) => (item.report?.evidence.length ?? 0) > 0).length, note: "Have photos/video", icon: "camera-outline" as const, tone: "blue" as const },
          { label: "To complete", value: base.length, note: "Not yet submitted", icon: "create-outline" as const, tone: "amber" as const },
        ]
      : [
          { label: "Submitted", value: base.filter((item) => item.status === "Submitted").length, note: "Awaiting review", icon: "paper-plane-outline" as const, tone: "blue" as const },
          { label: "Approved", value: base.filter((item) => item.status === "Approved").length, note: "Cleared by REA", icon: "checkmark-circle-outline" as const, tone: "green" as const },
...
function SyncScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={[styles.scrollContent, styles.listContent]} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.metricsGrid}>
        <KpiCard label="Uploading" value={uploading.length || (busy ? 1 : 0)} note="In progress" icon="cloud-upload-outline" tone="blue" />
        <KpiCard label="Waiting" value={waiting.length} note="Queued to sync" icon="time-outline" tone="amber" />
        <KpiCard label="Completed" value={completed.length} note="Safely uploaded" icon="checkmark-circle-outline" tone="green" />
      </View>
...
function InspectionModal({ assignment, onClose }: { assignment: Assignment | null; onClose: () => void }) {
  ...
  const sections = live ? getFormSections(live.component) : [];
  const formUnavailable = Boolean(live) && sections.length === 0;
...
function toneColor(tone: "green" | "amber" | "blue" | "violet") {
  return tone === "amber" ? colors.amber : tone === "blue" ? colors.blue : tone === "violet" ? colors.violet : colors.primary;
}
function tonePale(tone: "green" | "amber" | "blue" | "violet") {
  return tone === "amber" ? colors.amberPale : tone === "blue" ? colors.bluePale : tone === "violet" ? colors.violetPale : colors.paleStrong;
}

// Shared KPI "glass" card used on Overview and, now, every other tab's
// summary strip: soft frosted-tint background, a glass highlight blob,
...
const styles = StyleSheet.create({
  ...
  listContent: { gap: 9 },
  header: { height: 70, backgroundColor: "rgba(255,255,255,0.72)", borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.6)", paddingHorizontal: 18, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  headerBrand: { flexDirection: "row", alignItems: "center", gap: 10 },
  headerLogo: { width: 36, height: 36, borderRadius: 18, backgroundColor: "rgba(255,255,255,0.9)" },
  ...
  avatar: { width: 38, height: 38, borderRadius: 19, backgroundColor: "rgba(255,255,255,0.7)", borderWidth: 1, borderColor: "rgba(255,255,255,0.85)", alignItems: "center", justifyContent: "center" },
  ...
  greetingBanner: { height: greetingBannerSpec.height, marginTop: 8, borderRadius: 26, backgroundColor: "#EAF8EF", borderWidth: 1, borderColor: "rgba(255,255,255,0.7)", overflow: "hidden", shadowColor: "#214C38", shadowOpacity: 0.08, shadowRadius: 16, shadowOffset: { width: 0, height: 6 }, elevation: 3 },
  ...
  nextCard: { backgroundColor: "rgba(255,255,255,0.86)", borderWidth: 1, borderColor: "rgba(255,255,255,0.8)", borderRadius: 28, padding: 18, gap: 12, shadowColor: "#214C38", shadowOpacity: 0.09, shadowRadius: 22, shadowOffset: { width: 0, height: 10 }, elevation: 4, overflow: "hidden" },
  listPanel: { backgroundColor: "rgba(255,255,255,0.82)", borderWidth: 1, borderColor: "rgba(255,255,255,0.75)", borderRadius: 28, padding: 16, gap: 9, shadowColor: "#214C38", shadowOpacity: 0.06, shadowRadius: 20, shadowOffset: { width: 0, height: 8 }, elevation: 2 },
  ...
  primaryButton: { minHeight: 48, paddingHorizontal: 18, borderRadius: 24, flexDirection: "row", gap: 7, alignItems: "center", justifyContent: "center", backgroundColor: colors.primary, shadowColor: colors.primary, shadowOpacity: 0.22, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 3 },
  primaryButtonText: { color: colors.white, fontSize: 12, fontWeight: "800" },
  flexButton: { flex: 1 },
  outlineButton: { minHeight: 48, paddingHorizontal: 16, borderRadius: 24, borderWidth: 1, borderColor: "rgba(185,215,196,0.9)", flexDirection: "row", gap: 6, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(255,255,255,0.7)" },
  outlineButtonText: { color: colors.primary, fontSize: 12, fontWeight: "800" },
  disabled: { opacity: 0.45 },
  assignmentCard: { backgroundColor: "rgba(255,255,255,0.82)", borderWidth: 1, borderColor: "rgba(255,255,255,0.75)", borderRadius: 22, padding: 13, gap: 8, marginBottom: 2, shadowColor: "#214C38", shadowOpacity: 0.05, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: 1 },
  assignmentCardCompact: { borderRadius: 18, paddingVertical: 10, paddingHorizontal: 11, shadowColor: "#214C38", shadowOpacity: 0.04, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 1 },
  ...
  badge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 9, height: 26, borderRadius: 13 },
  badgeDot: { width: 5, height: 5, borderRadius: 3 },
  badgeText: { fontSize: 9, fontWeight: "800" },
  chipRow: { gap: 7, paddingVertical: 2 },
  chip: { borderWidth: 1, borderColor: "rgba(219,229,223,0.9)", backgroundColor: "rgba(255,255,255,0.75)", paddingHorizontal: 13, height: 34, borderRadius: 17, alignItems: "center", justifyContent: "center" },
  ...
  tabBar: { height: 72, marginHorizontal: 14, marginBottom: 10, padding: 7, backgroundColor: "rgba(255,255,255,0.82)", borderWidth: 1, borderColor: "rgba(255,255,255,0.85)", borderRadius: 30, flexDirection: "row", shadowColor: "#173B2A", shadowOpacity: 0.14, shadowRadius: 22, shadowOffset: { width: 0, height: 10 }, elevation: 7 },
  ...
  syncBanner: { flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: colors.bluePale, borderRadius: 24, padding: 15, borderWidth: 1, borderColor: "rgba(255,255,255,0.6)" },
  ...
  syncNowButton: { backgroundColor: colors.primary, paddingHorizontal: 16, height: 36, borderRadius: 18, alignItems: "center", justifyContent: "center" },
  syncNowButtonText: { color: colors.white, fontSize: 11, fontWeight: "800" },
  queueItemCard: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "rgba(255,255,255,0.82)", borderRadius: 20, padding: 12, borderWidth: 1, borderColor: "rgba(255,255,255,0.75)", shadowColor: "#214C38", shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 1 },
  syncStatusPill: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12 },
  ...
  modalHeader: { height: 64, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: "rgba(221,233,225,0.7)", flexDirection: "row", alignItems: "center", gap: 9, backgroundColor: "rgba(255,255,255,0.92)" },
  iconButton: { width: 38, height: 38, borderRadius: 19, alignItems: "center", justifyContent: "center", backgroundColor: colors.pale },
  ...
  gpsCard: { flexDirection: "row", alignItems: "center", gap: 11, backgroundColor: "rgba(255,255,255,0.9)", borderWidth: 1, borderColor: "rgba(221,233,225,0.8)", borderRadius: 20, padding: 14 },
  gpsIcon: { width: 44, height: 44, borderRadius: 22, backgroundColor: colors.paleStrong, alignItems: "center", justifyContent: "center" },
  gpsCopy: { flex: 1 },
  ...
  step: { width: 30, height: 30, borderRadius: 15, backgroundColor: "#e7ede9", alignItems: "center", justifyContent: "center" },
  stepActive: { backgroundColor: colors.primary },
  stepText: { color: colors.muted, fontSize: 10, fontWeight: "800" },
  stepTextActive: { color: colors.white },
  formCard: { backgroundColor: "rgba(255,255,255,0.92)", borderWidth: 1, borderColor: "rgba(221,233,225,0.8)", borderRadius: 20, padding: 15, gap: 2 },
  ...
  evidenceItem: { width: 170, height: 132, borderRadius: 16, overflow: "hidden", backgroundColor: colors.pale },
  ...
  captureButton: { width: 108, height: 132, borderRadius: 16, borderWidth: 1, borderStyle: "dashed", borderColor: "#8bcba0", backgroundColor: colors.pale, alignItems: "center", justifyContent: "center", gap: 5 },
  ...
  lockedCard: { flexDirection: "row", alignItems: "center", gap: 9, padding: 13, backgroundColor: colors.paleStrong, borderRadius: 16 },
  ...
  backHeader: { height: 70, backgroundColor: "rgba(255,255,255,0.78)", borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.6)", paddingHorizontal: 10, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  ...
  searchBar: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "rgba(255,255,255,0.75)", borderRadius: 22, paddingHorizontal: 15, height: 46, borderWidth: 1, borderColor: "rgba(255,255,255,0.8)" },
  ...
  logoutRow: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, height: 48, borderRadius: 24, backgroundColor: colors.redPale, marginTop: 4 },
  ...
  brandMark: { width: 92, height: 92, borderRadius: 46, backgroundColor: "rgba(255,255,255,0.9)" },
  ...
  loginCard: { alignSelf: "stretch", marginTop: 28, backgroundColor: "rgba(255,255,255,0.92)", borderWidth: 1, borderColor: "rgba(255,255,255,0.8)", borderRadius: 26, padding: 20, shadowColor: "#214C38", shadowOpacity: 0.08, shadowRadius: 20, shadowOffset: { width: 0, height: 8 }, elevation: 3 },
