...
export const sendReaUserPasswordResetApi = (id: string) => reaCall(`/users/${encodeURIComponent(id)}/password-reset`, { method: "POST" });
export const requestPasswordResetApi = (email: string) => fetch("/api/auth/forgot-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) }).then(async (response) => { const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Unable to request a password reset."); return payload; });
export const completePasswordResetApi = (token: string, password: string) => fetch("/api/auth/reset-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, password }) }).then(async (response) => { const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || "Unable to reset the password."); return payload; });
