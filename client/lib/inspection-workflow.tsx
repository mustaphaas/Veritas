...
function reportForApi(report: InspectionReport) {
  return {
    ...report,
    evidence: report.evidence.map(({ previewUrl: _previewUrl, ...item }) => item),
  };
}
...
async function uploadRemoteEvidence(
  ...
  if (!evidence.previewUrl || evidence.previewUrl.startsWith("/api/evidence/")) {
    return evidence;
  }
...
export function InspectionWorkflowProvider({
  ...
  const assignProject = useCallback(
    (project: Project, officer: string, dueDate: string) => {
      ...
      void (async () => {
        ...
        setAssignments((current) =>
          current.map((item) =>
            item.id === assignment.id ? { ...item, syncStatus: "queued" } : item,
          ),
        );
      });
      return assignment;
  ...
  const resetDemo = useCallback(() => void refreshFromBackend(), [refreshFromBackend]);
