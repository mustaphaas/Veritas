export type Project = {
  ...
  x: number;
  y: number;
  /** Real D1 site coordinates when available. */
  latitude?: number;
  longitude?: number;
};
...
export const stateProjectTargets = {
  Abia: 8, Adamawa: 11, "Akwa Ibom": 9, Anambra: 14, Bauchi: 13, Bayelsa: 5, Benue: 12, Borno: 10, "Cross River": 9, Delta: 15, Ebonyi: 7, Edo: 13, Ekiti: 6, Enugu: 11, FCT: 8, Gombe: 10, Imo: 9, Jigawa: 12, Kaduna: 16, Kano: 20, Katsina: 15, Kebbi: 8, Kogi: 9, Kwara: 10, Lagos: 18, Nasarawa: 11, Niger: 14, Ogun: 15, Ondo: 8, Osun: 9, Oyo: 14, Plateau: 12, Rivers: 16, Sokoto: 11, Taraba: 7, Yobe: 8, Zamfara: 9,
} as const;
...
export const projects: Project[] = Object.entries(stateProjectTargets).flatMap(([state, count], stateIndex) =>
  Array.from({ length: count }, (_, projectIndex) => {
    ...
    const component = firstStateProject ? "Mini Grid" : components[(seed + projectIndex * 2 + stateIndex) % components.length];
    return {
...
export const filterDefaults = { programs: "All Programmes", components: "All Components", states: "All States", contractors: "All Contractors", months: "All Months" } as const;
export type FilterKey = keyof typeof filterDefaults;
export type Filters = Record<FilterKey, string>;
export const defaultFilters: Filters = { ...filterDefaults };
export const filterLabels: Record<FilterKey, string> = { programs: "Programme", components: "Component", states: "State", contractors: "Contractor", months: "Month" };

export function matchingProjects(filters: Filters, ignore?: FilterKey, sourceProjects: Project[] = projects) {
...
export function getFilterOptions(filters: Filters, key: FilterKey, sourceProjects: Project[] = projects) {
  const values = [...new Set(matchingProjects(filters, key, sourceProjects).map((project) =>
    key === "programs" ? project.programme : key === "components" ? project.component : key === "states" ? project.state : key === "contractors" ? project.contractor : project.month,
  ))];
  return [filterDefaults[key], ...values];
}

export function summarizeProjectsByState(filteredProjects: Project[]): StateSummary[] {
  return [...new Set(filteredProjects.map((project) => project.state))].map((state) => {
    const stateProjects = filteredProjects.filter((project) => project.state === state);
    const stateComponents = [...new Set(stateProjects.map((project) => project.component))];
    return {
      state,
      projects: stateProjects.length,
      programmes: new Set(stateProjects.map((project) => project.programme)).size,
      components: stateComponents,
      byComponent: stateComponents.map((component) => ({ name: component, value: stateProjects.filter((project) => project.component === component).length })),
      kw: stateProjects.reduce((sum, project) => sum + project.kw, 0),
      households: stateProjects.reduce((sum, project) => sum + project.households, 0),
...
export function summarizePortfolio(filteredProjects: Project[]) {
  ...
  return {
    ...
    households: filteredProjects.reduce((total, project) => total + project.households, 0),
    verified,
    pending: filteredProjects.length - verified,
    verificationRate: filteredProjects.length ? Math.round((verified / filteredProjects.length) * 100) : 0,
  };
}
