...
function ConsultantCoverageMap({ assignments }: { assignments: InspectionAssignment[] }) {
  ...
  return (
    <div className="bg-white">
      <div className="grid lg:grid-cols-[minmax(0,1fr)_250px]">
        <div className="relative min-h-[390px] overflow-hidden bg-[#f8fbf9] p-3">
            ...
            <p className="mt-1 text-[9px] text-slate-500">
              {!selectedState
                ...
                : !selectedLga
                  ...
                  : "Pins use inspection, verified arrival, or stored project GPS coordinates"}
            </p>
            {selectedState && (
              <button
                type="button"
          ...
          {!stateFeatures.length ? (
            ...
            <svg viewBox={`0 0 ${NATIONAL_VIEW.width} ${NATIONAL_VIEW.height}`} className="h-[390px] w-full" role="img" aria-label="Nigeria consultant project coverage by state">
              ...
              <ProjectDots assignments={filteredAssignments} projector={nationalProjector} selectedId={selectedProject?.id} onSelect={openProject} />
            </svg>
          ) : !selectedLga ? (
            <svg viewBox={`0 0 ${DETAIL_VIEW.width} ${DETAIL_VIEW.height}`} className="h-[390px] w-full" role="img" aria-label={`${selectedState} consultant projects by local government`}>
              ...
              <ProjectDots assignments={stateAssignments} projector={stateProjector} selectedId={selectedProject?.id} onSelect={openProject} />
            </svg>
          ) : (
            <svg viewBox={`0 0 ${DETAIL_VIEW.width} ${DETAIL_VIEW.height}`} className="h-[390px] w-full" role="img" aria-label={`${selectedLga} project locations`}>
              {selectedLgaFeature && (
                <path d={pathForFeature(selectedLgaFeature, lgaProjector)} fill="#e5f3e9" stroke="#6cad80" strokeWidth="1.5" />
              )}
              <ProjectDots assignments={lgaAssignments} projector={lgaProjector} selectedId={selectedProject?.id} onSelect={setSelectedProject} />
            </svg>
          )}
        </div>
        ...
        <aside className="max-h-[414px] overflow-y-auto border-t border-slate-100 bg-white p-3 lg:border-l lg:border-t-0">
          <p className="mb-2 text-[9px] font-black uppercase tracking-[0.12em] text-slate-500">
            {selectedLga ? `${selectedLga} projects` : selectedState ? `${selectedState} projects` : "Consultant portfolio"}
          </p>
            ...
            {visibleList.slice(0, 40).map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full rounded-lg border p-2.5 text-left transition ${selectedProject?.id === item.id ? "border-[#79be91] bg-[#eff9f2]" : "border-slate-100 hover:border-[#cfe5d5] hover:bg-[#fbfefc]"}`}
                >
      ...
      {selectedProject && (
        <div className="grid gap-2 border-t border-slate-100 bg-[#fbfefc] px-4 py-3 text-[9px] text-slate-500 sm:grid-cols-2 lg:grid-cols-7">
          ...
          <div><span className="block font-bold uppercase text-slate-400">LGA</span><strong className="mt-1 block text-[10px] text-[#173b2a]">{selectedProject.lga}</strong></div>
...
export default function ConsultantCoverageMapEnhancer() {
  ...
  useEffect(() => {
    ...
    const attach = () => {
      ...
      if (subtitle) {
        subtitle.textContent = "Interactive Nigeria → State → LGA coverage with project pins plotted from project GPS coordinates";
      }
