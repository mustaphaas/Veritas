...
import { consultantMetrics, readConsultants } from "../lib/consultants";
...
export default function ReaAnalyticsDashboard({ projects=[] }:Props) {
  ...
  const monthly=months.map((month)=>{
    ...
    return {month:month.slice(0,3),submitted:rows.filter((p)=>p.status==="Submitted"||p.verified).length,verified:rows.filter((p)=>p.verified).length,pending:rows.filter((p)=>!p.verified).length,capacity:Number((rows.reduce((s,p)=>s+p.kw,0)/1000).toFixed(1)),households:rows.reduce((s,p)=>s+p.households,0)};
  });
  ...
  const funnel=[
    {label:"Pending",value:filtered.filter((p)=>p.status==="Pending").length,color:"#f59e0b"},
    {label:"In Progress",value:filtered.filter((p)=>p.status==="In progress").length,color:"#3b82f6"},
    {label:"Submitted",value:filtered.filter((p)=>p.status==="Submitted").length,color:"#8b5cf6"},
    {label:"Approved",value:filtered.filter((p)=>p.verified).length,color:"#10b981"},
    {label:"Returned",value:filtered.filter((p)=>p.status==="Returned").length,color:"#f43f5e"},
  ];
  const funnelMax=Math.max(1,...funnel.map((item)=>item.value));
  const contractors=Array.from(new Set(filtered.map((p)=>p.contractor))).map((name)=>{
    const rows=filtered.filter((p)=>p.contractor===name);
    const completed=rows.filter((p)=>p.verified).length;
    const defects=rows.filter((p)=>p.status==="Pending").length;
    const reinspections=rows.filter((p)=>p.status==="Returned").length;
    const completionRate=rows.length?Math.round(completed/rows.length*100):0;
    const defectRate=rows.length?Math.round(defects/rows.length*100):0;
    const reinspectionRate=rows.length?Math.round(reinspections/rows.length*100):0;
    const quality=Math.max(0,Math.round(100-defectRate*.7-reinspectionRate*.9));
    return {name,projects:rows.length,completionRate,defectRate,reinspectionRate,quality,trend:completionRate>=60?"Improving":completionRate>=45?"Stable":"Needs attention"};
  }).sort((a,b)=>b.quality-a.quality);
  const consultants=readConsultants().map((record)=>({record,metrics:consultantMetrics(record)}));
  ...
  return <div className="relative space-y-4 overflow-hidden py-4">
    ...
    <section className="grid gap-4 xl:grid-cols-[1.55fr_.75fr]">
      <motion.article initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-lg">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2"><TrendingUp className="h-4 w-4 text-blue-600"/><h3 className="text-sm font-black text-[#173b2a]">Project & Verification Trend</h3></div><p className="mt-1 text-[10px] text-slate-500">Submitted reports compared with verified reports over time</p></div><div className="flex gap-3 text-[9px] font-bold text-slate-500"><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-blue-500"/>Submitted</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-emerald-500"/>Verified</span></div></div>
        <div className="mt-5 h-[330px]">{filtered.length?<ResponsiveContainer width="100%" height="100%"><ComposedChart data={monthly}><defs><linearGradient id="deliveryArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3b82f6" stopOpacity=".35"/><stop offset="100%" stopColor="#3b82f6" stopOpacity=".02"/></linearGradient></defs><CartesianGrid vertical={false} stroke="#eef2f7" strokeDasharray="4 5"/><XAxis dataKey="month" axisLine={false} tickLine={false} tick={{fontSize:10,fill:"#64748b"}}/><YAxis axisLine={false} tickLine={false} allowDecimals={false} tick={{fontSize:10,fill:"#94a3b8"}}/><Tooltip content={<ChartTooltip/>}/><Area type="monotone" dataKey="submitted" name="Submitted" stroke="#3b82f6" strokeWidth={3} fill="url(#deliveryArea)" animationDuration={1200}/><Line type="monotone" dataKey="verified" name="Verified" stroke="#10b981" strokeWidth={3} dot={{r:3,fill:"#fff",strokeWidth:2}} animationDuration={1500}/></ComposedChart></ResponsiveContainer>:<EmptyChart/>}</div>
      </motion.article>
    ...
    <section className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-lg"><div className="flex items-center justify-between"><div><div className="flex items-center gap-2"><BarChart3 className="h-4 w-4 text-violet-600"/><h3 className="text-sm font-black text-[#173b2a]">Programme Performance</h3></div><p className="mt-1 text-[10px] text-slate-500">Projects and installed capacity by programme</p></div><span className="rounded-full bg-violet-50 px-3 py-1 text-[9px] font-bold text-violet-700">{byProgramme.length} programmes</span></div><div className="mt-4 h-[290px]">{byProgramme.length?<ResponsiveContainer width="100%" height="100%"><BarChart data={byProgramme}><CartesianGrid vertical={false} stroke="#eef2f7" strokeDasharray="4 5"/><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize:10}}/><YAxis axisLine={false} tickLine={false} allowDecimals={false} tick={{fontSize:10}}/><Tooltip content={<ChartTooltip/>}/><Bar dataKey="projects" name="Projects" fill="#8b5cf6" radius={[7,7,0,0]} animationDuration={1000}/><Bar dataKey="capacity" name="Capacity (MW)" fill="#10b981" radius={[7,7,0,0]} animationDuration={1400}/></BarChart></ResponsiveContainer>:<EmptyChart/>}</div></article>
    ...
    <section className="grid gap-4 xl:grid-cols-[.72fr_1.28fr]">
      <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-lg">
        <div className="flex items-center gap-2"><Filter className="h-4 w-4 text-amber-600"/><h3 className="text-sm font-black text-[#173b2a]">Inspection Status Funnel</h3></div>
        <p className="mt-1 text-[10px] text-slate-500">Identify where the inspection and approval pipeline is clogging</p>
        <div className="mt-6 space-y-3">{funnel.map((item,index)=>{const width=Math.max(12,item.value/funnelMax*100-index*2);return <div key={item.label}><div className="mb-1.5 flex items-center justify-between text-[10px]"><span className="font-bold text-slate-600">{item.label}</span><strong className="text-slate-800">{item.value}</strong></div><div className="h-9 overflow-hidden rounded-lg bg-slate-100"><motion.div initial={{width:0}} animate={{width:width+"%"}} transition={{duration:.8,delay:index*.1}} className="flex h-full items-center justify-end rounded-lg px-3 text-[9px] font-black text-white" style={{backgroundColor:item.color}}>{Math.round(item.value/Math.max(1,filtered.length)*100)}%</motion.div></div></div>})}</div>
      </article>
      <article className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-lg">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-5"><div><div className="flex items-center gap-2"><ShieldAlert className="h-4 w-4 text-rose-600"/><h3 className="text-sm font-black text-[#173b2a]">Contractor Performance</h3></div><p className="mt-1 text-[10px] text-slate-500">Delivery, defects, re-inspection and quality accountability</p></div><span className="rounded-full bg-rose-50 px-3 py-1 text-[9px] font-bold text-rose-700">{contractors.length} contractors</span></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead className="bg-slate-50 text-[9px] uppercase tracking-wider text-slate-500"><tr><th className="px-4 py-3">Contractor</th><th className="px-4 py-3">Projects</th><th className="px-4 py-3">Completion</th><th className="px-4 py-3">Defect rate</th><th className="px-4 py-3">Re-inspection</th><th className="px-4 py-3">Quality</th><th className="px-4 py-3">Trend</th></tr></thead><tbody>{contractors.map((item)=><tr key={item.name} className="border-t border-slate-100"><td className="px-4 py-3 text-xs font-bold text-[#173b2a]">{item.name}</td><td className="px-4 py-3 text-xs font-semibold">{item.projects}</td><td className="px-4 py-3"><span className="text-xs font-black text-emerald-700">{item.completionRate}%</span></td><td className="px-4 py-3 text-xs font-bold text-amber-600">{item.defectRate}%</td><td className="px-4 py-3 text-xs font-bold text-rose-600">{item.reinspectionRate}%</td><td className="px-4 py-3"><div className="flex items-center gap-2"><div className="h-1.5 w-16 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-violet-500" style={{width:item.quality+"%"}}/></div><strong className="text-[10px]">{item.quality}/100</strong></div></td><td className="px-4 py-3"><span className={"rounded-full px-2 py-1 text-[9px] font-bold "+(item.trend==="Improving"?"bg-emerald-50 text-emerald-700":item.trend==="Stable"?"bg-blue-50 text-blue-700":"bg-rose-50 text-rose-700")}>{item.trend}</span></td></tr>)}</tbody></table></div>
        <p className="border-t border-slate-100 bg-slate-50 px-5 py-3 text-[9px] text-slate-500">Quality signals currently use inspection status data; production defect and re-inspection events should come from the verification database.</p>
      </article>
    </section>

    <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-lg">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-5"><div><div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-violet-600"/><h3 className="text-sm font-black text-[#173b2a]">Consultant Performance</h3></div><p className="mt-1 text-[10px] text-slate-500">QA approval behaviour, turnaround and re-inspection requests</p></div><span className="rounded-full bg-violet-50 px-3 py-1 text-[9px] font-bold text-violet-700">{consultants.length} consultant firms</span></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead className="bg-slate-50 text-[9px] uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">Consultant</th><th className="px-5 py-3">Coverage</th><th className="px-5 py-3">Reviewed</th><th className="px-5 py-3">Approval rate</th><th className="px-5 py-3">Avg. turnaround</th><th className="px-5 py-3">Re-inspection requests</th><th className="px-5 py-3">Status</th></tr></thead><tbody>{consultants.map(({record,metrics})=><tr key={record.id} className="border-t border-slate-100"><td className="px-5 py-4"><p className="text-xs font-bold text-[#173b2a]">{record.firmName}</p><p className="mt-1 text-[9px] text-slate-400">{record.adminName}</p></td><td className="px-5 py-4 text-[10px] text-slate-600">{record.regions.join(", ")||"—"}</td><td className="px-5 py-4 text-xs font-bold">{metrics.reviewed}</td><td className="px-5 py-4"><strong className="text-xs text-emerald-700">{metrics.approvalRate}%</strong></td><td className="px-5 py-4 text-xs font-bold text-blue-700">{metrics.averageTurnaroundHours}h</td><td className="px-5 py-4 text-xs font-bold text-rose-600">{metrics.reinspectionRate}%</td><td className="px-5 py-4"><span className={"rounded-full px-2 py-1 text-[9px] font-bold "+(record.status==="Active"?"bg-emerald-50 text-emerald-700":"bg-amber-50 text-amber-700")}>{record.status}</span></td></tr>)}</tbody></table></div>
    </section>
