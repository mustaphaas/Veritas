...
const selectClass = "h-10 min-w-0 rounded-xl border border-white/20 bg-white/10 px-3 text-xs font-semibold text-white outline-none backdrop-blur-md transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-300/20 [&>option]:text-slate-800";
...
export default function ReaAnalyticsDashboard({ projects=[] }:Props) {
  ...
  return <div className="relative space-y-4 overflow-hidden py-4">
    <motion.section initial={{opacity:0,y:-16}} animate={{opacity:1,y:0}} className="relative overflow-hidden rounded-2xl bg-[#072f24] p-5 text-white shadow-2xl shadow-emerald-950/20 lg:p-7">
      <div className="pointer-events-none absolute -right-16 -top-24 h-72 w-72 rounded-full bg-emerald-400/20 blur-3xl"/>
      <div className="pointer-events-none absolute bottom-[-90px] left-[32%] h-56 w-56 rounded-full bg-blue-500/20 blur-3xl"/>
      <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
        <div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-[9px] font-bold uppercase tracking-[.22em] text-emerald-200"><Sparkles className="h-3 w-3"/>Live intelligence</div><h2 className="text-2xl font-black tracking-tight sm:text-3xl">National Delivery Analytics</h2><p className="mt-2 max-w-xl text-xs leading-5 text-emerald-50/65">Explore programme delivery, verification performance and geographic impact across Nigeria.</p></div>
        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><select aria-label="Programme" value={programme} onChange={(e)=>setProgramme(e.target.value)} className={selectClass}>{options.programmes.map((p)=><option key={p}>{p}</option>)}</select><select aria-label="Component" value={component} onChange={(e)=>setComponent(e.target.value)} className={selectClass}>{options.components.map((p)=><option key={p}>{p}</option>)}</select><select aria-label="State" value={state} onChange={(e)=>setState(e.target.value)} className={selectClass}>{options.states.map((p)=><option key={p}>{p}</option>)}</select><button onClick={()=>exportCsv(filtered)} disabled={!filtered.length} className="flex h-10 items-center justify-center gap-2 rounded-xl bg-white px-4 text-xs font-bold text-[#073b2b] shadow-lg transition hover:-translate-y-0.5 disabled:opacity-50"><Download className="h-4 w-4"/>Export data</button></div>
      </div>
      <div className="relative mt-5 flex flex-wrap items-center gap-2 border-t border-white/10 pt-4"><Filter className="mr-1 h-4 w-4 text-emerald-300"/>{(["All","H1","H2"] as Period[]).map((p)=><button key={p} onClick={()=>setPeriod(p)} className={"rounded-lg px-3 py-2 text-[10px] font-bold transition "+(period===p?"bg-emerald-400 text-emerald-950 shadow-lg shadow-emerald-500/20":"bg-white/5 text-white/65 hover:bg-white/10")}>{p==="All"?"Full year":p}</button>)}<button onClick={reset} className="ml-auto flex items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-[10px] font-bold text-white/70 transition hover:bg-white/10"><RefreshCcw className="h-3.5 w-3.5"/>Reset filters</button></div>
    </motion.section>
