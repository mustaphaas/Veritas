...
export default function ReaConsultantsManagement() {
  ...
  return <div className="space-y-4 py-4">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {[["Total Consultants",records.length,Building2,"bg-emerald-50 text-emerald-700"],["Active",records.filter((r)=>r.status==="Active").length,CheckCircle2,"bg-blue-50 text-blue-700"],["Pending Activation",records.filter((r)=>r.status==="Pending Activation").length,Clock3,"bg-amber-50 text-amber-700"],["Covered States",new Set(records.flatMap((r)=>Array.isArray(r.states)?r.states:[])).size,MapPin,"bg-violet-50 text-violet-700"]].map(([label,value,Icon,tone]:any)=><article key={label} className="group min-h-[104px] rounded-lg border border-slate-200 bg-white p-4 text-left shadow-[0_1px_2px_rgba(16,24,40,0.04)] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#9dceb0] hover:shadow-md"><div className="flex h-full items-start gap-3"><div className={"flex h-10 w-10 shrink-0 items-center justify-center rounded-lg transition-all duration-200 group-hover:bg-[#08733f] group-hover:text-white "+tone}><Icon className="h-5 w-5 transition-transform duration-200 group-hover:scale-110"/></div><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-[#263c31]">{label}</p><p className="mt-1 text-[23px] font-bold leading-none tracking-tight text-[#13281e]">{value}</p><p className="mt-2 text-[11px] text-slate-500">Current consultant portfolio</p></div></div></article>)}
    </section>
    {notice&&<p className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs font-semibold text-emerald-700">{notice}</p>}
