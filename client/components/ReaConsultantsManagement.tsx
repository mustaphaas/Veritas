...
import {
  appendConsultantActivity, consultantMetrics, readConsultantActivity,
...
function safeAccounts() {
  try { const records = readConsultants(); return Array.isArray(records) ? records : []; }
  catch { return []; }
}
...
export default function ReaConsultantsManagement() {
  ...
  const filtered = useMemo(()=>records.filter((r)=>{
    ...
    const matches=!q || (r.firmName+" "+r.adminName+" "+r.adminEmail+" "+r.regions.join(" ")+" "+r.states.join(" ")).toLowerCase().includes(q);
