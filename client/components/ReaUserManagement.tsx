...
export default function ReaUserManagement() {
  ...
  async function changeStatus(user: PortalUser, status: "Active" | "Suspended") {
    try { await updateReaUserStatusApi(user.id, status); setSuccess(`${user.name} is now ${status}.`); loadUsers(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to update user status."); }
  }
  async function deleteUser(user: PortalUser) {
    if (!window.confirm(`Delete ${user.name}? This cannot be undone.`)) return;
    try { await deleteReaUserApi(user.id); setSuccess(`${user.name} was deleted.`); loadUsers(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to delete user."); }
  }
  async function saveAccess() {
    if (!actionUser) return;
    try { await updateReaUserAccessApi(actionUser.id, accessDraft); setActionUser(null); setSuccess("Dashboard access updated."); loadUsers(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to update access."); }
  }
  async function resetPassword() {
    if (!actionUser || newPassword.length < 8) { setError("Password must be at least 8 characters."); return; }
    try { await resetReaUserPasswordApi(actionUser.id, newPassword); setActionUser(null); setSuccess("Password reset successfully."); loadUsers(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to reset password."); }
  }
  ...
  return <div className="space-y-4 pb-8 pt-4">
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[980px] table-fixed text-left"><thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">User Name</th><th className="px-4 py-3">Email</th><th className="px-4 py-3">Classification</th><th className="px-4 py-3">Role / Department</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Access</th></tr></thead>
        <tbody>{visible.map((user)=><tr key={user.id} className="border-t border-slate-100 hover:bg-[#f8fcf9]"><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-50 text-xs font-bold text-emerald-700">{user.name.split(/\s+/).slice(0,2).map((part)=>part[0]).join("")}</span><p className="text-xs font-bold text-slate-800">{user.name}</p></div></td><td className="px-4 py-4 text-xs text-slate-600">{user.email || "—"}</td><td className="px-4 py-4"><span className="rounded-full bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-700">{user.classification}</span></td><td className="px-4 py-4"><p className="text-xs font-semibold text-slate-700">{roleLabel(user.role)}</p><p className="mt-0.5 text-[10px] text-slate-500">{user.department || user.consultantFirm || (user.classification === "REA Staff" ? "Rural Electrification Agency" : "—")}</p></td><td className="px-4 py-4"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${user.status === "Active" ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{user.status}</span></td><td className="relative px-4 py-4 text-[10px] text-slate-500">{user.classification === "REA Staff" ? (user.access?.length || 0) + " modules" : "—"} <button type="button" onClick={()=>setActionMenu(actionMenu===user.id?null:user.id)} className="ml-2 rounded p-1 text-slate-400 hover:bg-slate-100"><MoreHorizontal className="h-4 w-4"/></button>{actionMenu===user.id && <div className="absolute right-4 z-20 mt-1 w-48 rounded-lg border border-slate-200 bg-white p-1 shadow-xl"><button className="block w-full rounded px-3 py-2 text-left text-xs hover:bg-slate-50" onClick={()=>{setActionUser(user);setAccessDraft(user.access||[]);setActionMenu(null)}}>Access control</button><button className="block w-full rounded px-3 py-2 text-left text-xs hover:bg-slate-50" onClick={()=>{setActionUser(user);setNewPassword("");setActionMenu(null)}}>Reset password</button>{user.status==="Active"?<button className="block w-full rounded px-3 py-2 text-left text-xs text-amber-700 hover:bg-amber-50" onClick={()=>void changeStatus(user,"Suspended")}>Suspend user</button>:<button className="block w-full rounded px-3 py-2 text-left text-xs text-emerald-700 hover:bg-emerald-50" onClick={()=>void changeStatus(user,"Active")}>Activate user</button>}<button className="block w-full rounded px-3 py-2 text-left text-xs text-rose-700 hover:bg-rose-50" onClick={()=>void deleteUser(user)}>Delete user</button></div>}</td></tr>)}</tbody></table>{visible.length === 0 && <div className="border-t border-slate-100 p-8 text-center text-xs text-slate-500">No database users match this view.</div>}</div>}
    </section>

    {actionUser && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4"><div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b p-5"><div><h3 className="font-bold text-[#173b2a]">Manage User</h3><p className="mt-1 text-xs text-slate-500">{actionUser.name}</p></div><button onClick={()=>setActionUser(null)}><X className="h-5 w-5 text-slate-400"/></button></div><div className="space-y-5 p-5"><div><p className="mb-2 text-xs font-bold text-slate-700">Dashboard access</p><div className="grid gap-2 sm:grid-cols-2">{reaAccessModules.map(module=><label key={module} className="flex items-center gap-2 rounded-lg border p-3 text-xs"><input type="checkbox" checked={accessDraft.includes(module)} onChange={()=>setAccessDraft(v=>v.includes(module)?v.filter(x=>x!==module):[...v,module])}/>{module}</label>)}</div></div><div className="border-t pt-4"><p className="mb-2 text-xs font-bold text-slate-700">Reset password</p><div className="flex gap-2"><input type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)} placeholder="New password (8+ characters)" className="h-10 flex-1 rounded-lg border px-3 text-xs"/><button onClick={()=>void resetPassword()} className="rounded-lg bg-[#08733f] px-3 text-xs font-bold text-white">Reset</button></div></div></div><div className="flex justify-end gap-3 border-t p-5"><button onClick={()=>void saveAccess()} className="rounded-lg bg-[#08733f] px-4 py-2.5 text-xs font-bold text-white">Save Access</button></div></div></div>}
