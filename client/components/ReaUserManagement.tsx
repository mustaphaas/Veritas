    ...
    {showCreate && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
        ...
        <div className="grid gap-4 p-6 sm:grid-cols-2">
          <Field label="Full name"><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="e.g. Musa Ibrahim" className="input"/></Field>
          <Field label="Email address"><input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} placeholder="name@rea.gov.ng" className="input"/></Field>
          <Field label="Phone number"><input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="Optional" className="input"/></Field>
          <Field label="Department"><input value={form.department} onChange={e=>setForm({...form,department:e.target.value})} placeholder="e.g. Programme Delivery" className="input"/></Field>
          <Field label="Role"><select value={form.staffRole} onChange={e=>setForm({...form,staffRole:e.target.value})} className="input">{STAFF_ROLES.map(role=><option key={role}>{role}</option>)}</select></Field>
          <Field label="Temporary password"><div className="flex gap-2"><input value={form.temporaryPassword} onChange={e=>setForm({...form,temporaryPassword:e.target.value})} className="input"/><button type="button" onClick={()=>setForm({...form,temporaryPassword:makePassword()})} className="rounded-lg border border-slate-200 px-3 text-[10px] font-bold text-slate-600">Generate</button></div></Field>
