...
const filterDefaults = { programs: "All Programmes", components: "All Components", states: "All States", contractors: "All Contractors", months: "Month" } as const;
type FilterKey = keyof typeof filterDefaults;
type Filters = Record<FilterKey, string>;
const defaultFilters: Filters = { ...filterDefaults };
const filterLabels: Record<FilterKey, string> = { programs: "Programme", components: "Component", states: "State", contractors: "Contractor", months: "Month" };
...
function matchingProjects(filters: Filters, ignore?: FilterKey) {
  return projects.filter((project) =>
    ...
    (ignore === "states" || filters.states === filterDefaults.states || project.state === filters.states) &&
...
function getFilterOptions(filters: Filters, key: FilterKey) {
  const values = [...new Set(matchingProjects(filters, key).map((project) => key === "programs" ? project.programme : key === "components" ? project.component : key === "states" ? project.state : key === "contractors" ? project.contractor : project.month))];
...
const trendBaseline = [
  { month: "Jan", inspections: 28, submitted: 23, verified: 16 },
  { month: "Feb", inspections: 31, submitted: 27, verified: 19 },
  { month: "Mar", inspections: 35, submitted: 31, verified: 23 },
  { month: "Apr", inspections: 39, submitted: 35, verified: 27 },
  { month: "May", inspections: 44, submitted: 39, verified: 31 },
  { month: "Jun", inspections: 48, submitted: 44, verified: 36 },
  { month: "Jul", inspections: 53, submitted: 48, verified: 41 },
  { month: "Aug", inspections: 58, submitted: 53, verified: 46 },
];
...
export default function Index() {
  ...
  const programmePerformance = useMemo(() => (Object.keys(programmeTotals) as Array<keyof typeof programmeTotals>).map((programme) => {
    const total = programmeTotals[programme];
    ...
    const share = matching.length / programmeProjects.length;
    return {
      programme,
      projects: Math.round(total.projects * share),
      capacity: total.capacity * share,
      households: Math.round(total.households * share),
      verified: Math.round(total.verified * share),
    };
  }).filter((row) => row.projects > 0), [visibleProjects]);
  const trendData = useMemo(() => {
    const scale = visibleProjects.length / projects.length;
    const selectedMonth = filters.months === filterDefaults.months ? null : filters.months.slice(0, 3);
    return trendBaseline.filter((point) => !selectedMonth || point.month === selectedMonth).map((point) => ({
      ...point,
      inspections: Math.round(point.inspections * scale),
      submitted: Math.round(point.submitted * scale),
      verified: Math.round(point.verified * scale),
    }));
  }, [visibleProjects, filters.months]);
      ...
      <main className="lg:pl-64">
        ...
        <div className="mx-auto max-w-[1520px] px-4 py-7 sm:px-7 lg:px-9">
          ...
          <section className="mt-7 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
            <div className="border-b border-slate-200 px-5 py-4"><h2 className="font-bold text-[#173b2a]">Programme Performance</h2><p className="mt-1 text-xs text-slate-500">Programme-level delivery for the current filtered view</p></div>
            <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left"><thead className="bg-slate-50 text-[10px] uppercase tracking-[0.1em] text-slate-500"><tr>{["Programme", "Projects", "Capacity", "Households", "Verified"].map((heading) => <th key={heading} className="px-5 py-3 font-semibold">{heading}</th>)}</tr></thead><tbody>{programmePerformance.map((row) => <tr key={row.programme} className="border-t border-slate-100"><td className="px-5 py-4 text-sm font-bold text-[#08733f]">{row.programme}</td><td className="px-5 py-4 text-sm font-semibold text-[#173b2a]">{row.projects}</td><td className="px-5 py-4 text-sm text-slate-600">{row.capacity.toFixed(1)} MW</td><td className="px-5 py-4 text-sm text-slate-600">{row.households.toLocaleString()}</td><td className="px-5 py-4"><span className="rounded-full bg-[#eaf8ef] px-2.5 py-1 text-xs font-bold text-[#08733f]">{row.verified}%</span></td></tr>)}</tbody></table></div>
          ...
          <section className="mt-7 rounded-lg border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
            <div><h2 className="font-bold text-[#173b2a]">Project &amp; Verification Trend</h2><p className="mt-1 text-xs text-slate-500">Monthly movement in inspections, submissions and REA verification</p></div>
            <div className="mt-5 h-64" aria-label="Project and verification trend chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData} margin={{ top: 8, right: 12, left: -20, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="#e5ece7" /><XAxis dataKey="month" tick={{ fontSize: 11, fill: "#64748b" }} axisLine={false} tickLine={false} /><YAxis allowDecimals={false} tick={{ fontSize: 11, fill: "#64748b" }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ borderRadius: 8, borderColor: "#dbe7de", fontSize: 12 }} /><Line type="monotone" dataKey="inspections" name="Inspections completed" stroke="#08733f" strokeWidth={2.5} dot={{ r: 3 }} /><Line type="monotone" dataKey="submitted" name="Reports submitted" stroke="#377fd2" strokeWidth={2.5} dot={{ r: 3 }} /><Line type="monotone" dataKey="verified" name="Reports verified" stroke="#d89100" strokeWidth={2.5} dot={{ r: 3 }} /></LineChart></ResponsiveContainer></div>
            <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-600">{[["#08733f", "Inspections completed"], ["#377fd2", "Reports submitted"], ["#d89100", "Reports verified"]].map(([color, label]) => <span key={label} className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: color }} />{label}</span>)}</div>
          </section>
