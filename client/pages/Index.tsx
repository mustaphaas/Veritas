...
export default function Index() {
  ...
  const navContent = (
    <>
      <div className="veritas-sidebar-brand"><AtlasMark /><div><p className="veritas-wordmark">VERITAS</p><p>REA project intelligence</p></div></div>
      <nav className="veritas-sidebar-nav"><p className="veritas-nav-label">Workspace</p>{visibleNavigation.map(({ label, icon: Icon }) => <button key={label} onClick={() => { setActiveNav(label); setMobileMenuOpen(false); }} className={resolvedActiveNav === label ? "is-active" : ""}><span className="veritas-nav-icon"><Icon strokeWidth={resolvedActiveNav === label ? 2.2 : 1.8} /></span><span>{label}</span><ArrowRight className="veritas-nav-arrow" /></button>)}</nav>
      <div className="veritas-sidebar-footer"><button><Settings /> Settings</button><div className="veritas-sidebar-profile"><span><UsersRound /></span><div><strong>{session?.name ?? "REA Administrator"}</strong><small>REA Administrator</small></div></div></div>
    </>
  );

  return (
    <div className="veritas-app-shell min-h-screen text-slate-900">
      <aside className="veritas-desktop-sidebar fixed inset-y-0 left-0 z-30 hidden w-[244px] flex-col lg:flex">{navContent}</aside>
      <div className={`fixed inset-0 z-50 lg:hidden ${mobileMenuOpen ? "" : "pointer-events-none"}`}><div onClick={() => setMobileMenuOpen(false)} className={`absolute inset-0 bg-slate-950/35 backdrop-blur-sm transition-opacity ${mobileMenuOpen ? "opacity-100" : "opacity-0"}`} /><aside className={`veritas-mobile-sidebar absolute inset-y-0 left-0 flex w-[290px] flex-col shadow-2xl transition-transform ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}><button onClick={() => setMobileMenuOpen(false)} className="absolute right-3 top-4 z-10 rounded-lg p-2 text-white/70 hover:bg-white/10 hover:text-white"><X className="h-5 w-5" /></button>{navContent}</aside></div>
      <main className="veritas-main lg:pl-[244px]">
        <header className="veritas-topbar sticky top-0 z-20 flex h-[82px] items-center justify-between px-4 backdrop-blur sm:px-7 lg:px-8"><div className="flex items-center gap-3"><button onClick={() => setMobileMenuOpen(true)} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100 lg:hidden" aria-label="Open navigation"><Menu className="h-5 w-5" /></button><div><p className="veritas-topbar-eyebrow">National monitoring workspace</p><h1 className="text-lg font-bold tracking-tight text-[#142a1f] sm:text-[22px]">{resolvedActiveNav || "REA Dashboard"}</h1></div></div><div className="flex items-center gap-2 sm:gap-3"><span className="veritas-live-pill hidden items-center gap-2 text-xs font-semibold md:flex"><i className="h-2 w-2 rounded-full" />Live data</span><button className="veritas-topbar-icon relative" aria-label="Notifications"><Bell className="h-5 w-5" /><span>3</span></button><button type="button" onClick={() => { logout(); navigate("/login", { replace: true }); }} className="veritas-logout"><LogOut className="h-4 w-4" /><span className="hidden xl:inline">Logout</span></button></div></header>
        <div className="veritas-content mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-8">
          {!resolvedActiveNav ? (
