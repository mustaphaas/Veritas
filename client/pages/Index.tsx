      ...
      <main className="lg:pl-[72px]">
        ...
        <div className="mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-7">
          ...
          <section className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <article className="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex items-start justify-between border-b border-slate-100 px-5 py-4"><div className="flex items-start gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-md bg-[#eaf8ef] text-[#08733f]"><Clock3 className="h-5 w-5" /></div><div><h2 className="text-sm font-bold text-[#173b2a]">Recent Activity</h2><p className="mt-1 text-xs text-slate-500">Latest updates across projects</p></div></div><button className="text-xs font-semibold text-[#08733f] hover:underline">View all activity</button></div><div className="px-5 py-2">{visibleProjects.slice(0, 2).map((project, index) => <div key={project.name} className="flex items-center gap-3 border-b border-slate-100 py-3 last:border-0"><span className={`h-2.5 w-2.5 shrink-0 rounded-full ${project.verified ? "bg-[#18a15b]" : project.status === "Pending" ? "bg-[#e6ad21]" : "bg-[#377fd2]"}`} /><div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md ${project.verified ? "bg-[#edf8f0] text-[#08733f]" : "bg-[#eef5fc] text-[#3772ad]"}`}>{project.verified ? <MapPin className="h-4 w-4" /> : <FileCheck2 className="h-4 w-4" />}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-[#173b2a]">{project.name}</p><p className="mt-0.5 text-[11px] text-slate-500">{project.verified ? "Inspection verified" : project.status === "Pending" ? "Report submitted for review" : "Application submitted"}</p></div><span className="shrink-0 text-[11px] text-slate-500">{index === 0 ? "Today, 10:24 AM" : `${index + 2} days ago`}</span></div>)}</div></article>
            <article className="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex items-start gap-3 border-b border-slate-100 px-5 py-4"><div className="flex h-9 w-9 items-center justify-center rounded-md bg-[#eaf8ef] text-[#08733f]"><Zap className="h-5 w-5" /></div><div><h2 className="text-sm font-bold text-[#173b2a]">Quick Actions</h2><p className="mt-1 text-xs text-slate-500">Do more with your projects</p></div></div><div className="grid grid-cols-2 gap-3 p-4"><button onClick={() => setActiveNav("Projects")} className="flex min-h-[92px] flex-col items-center justify-center rounded-lg border border-[#cdebd6] bg-[#effaf2] p-3 text-center hover:bg-[#e6f7eb]"><FolderKanban className="h-6 w-6 text-[#08733f]" /><strong className="mt-2 block text-[11px] text-[#173b2a]">Add Project</strong></button><button className="flex min-h-[92px] flex-col items-center justify-center rounded-lg border border-[#d7e4f5] bg-[#f1f6fd] p-3 text-center hover:bg-[#eaf2fc]"><Download className="h-6 w-6 text-[#3772ad]" /><strong className="mt-2 block text-[11px] text-[#173b2a]">Upload Report</strong></button><button onClick={() => setActiveNav("Claims")} className="flex min-h-[92px] flex-col items-center justify-center rounded-lg border border-[#dddff7] bg-[#f5f4fd] p-3 text-center hover:bg-[#eeecfb]"><UsersRound className="h-6 w-6 text-[#6078d3]" /><strong className="mt-2 block text-[11px] text-[#173b2a]">Assign Inspector</strong></button><button onClick={() => setActiveNav("Claims")} className="flex min-h-[92px] flex-col items-center justify-center rounded-lg border border-[#f3dfad] bg-[#fff8e8] p-3 text-center hover:bg-[#fff3d5]"><Clock3 className="h-6 w-6 text-[#d89100]" /><strong className="mt-2 block text-[11px] text-[#173b2a]">View Pending ({visibleProjects.filter((project) => !project.verified).length})</strong></button></div></article>
            <article className="flex flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm md:col-span-2 xl:col-span-1"><div className="flex items-center justify-between gap-3"><h2 className="text-sm font-bold text-[#173b2a]">Verification Status</h2><span className="text-[10px] font-semibold text-slate-500">Filtered portfolio</span></div><div className="mt-5 flex items-end justify-between gap-3"><div><p className="text-4xl font-bold tracking-tight text-[#173b2a]">{visibleProjects.length ? Math.round((visibleProjects.filter((project) => project.verified).length / visibleProjects.length) * 100) : 0}%</p><p className="mt-1 text-[11px] text-slate-500">{visibleProjects.filter((project) => project.verified).length} of {visibleProjects.length} reports verified</p></div><CheckCircle2 className="h-9 w-9 text-[#119653]" /></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#119653] transition-all duration-700" style={{ width: `${visibleProjects.length ? (visibleProjects.filter((project) => project.verified).length / visibleProjects.length) * 100 : 0}%` }} /></div><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-lg bg-[#edf8f0] p-3 text-center"><p className="text-2xl font-bold text-[#08733f]">{visibleProjects.filter((project) => project.verified).length}</p><p className="mt-1 text-[10px] font-semibold text-[#39764d]">Verified</p></div><div className="rounded-lg bg-[#fff7e3] p-3 text-center"><p className="text-2xl font-bold text-[#c88400]">{visibleProjects.filter((project) => !project.verified).length}</p><p className="mt-1 text-[10px] font-semibold text-[#8a6721]">Pending</p></div></div><button onClick={() => setActiveNav("Claims")} className="mt-4 flex w-full items-center justify-between rounded-md border border-[#efdb9d] bg-[#fffaf0] px-3 py-2.5 text-[11px] font-bold text-[#9a6500] hover:bg-[#fff5dc]">Review verification queue<ArrowRight className="h-4 w-4" /></button></article>
          </section>
