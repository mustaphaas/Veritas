        ...
        <div className="mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-7">
          ...
          <div className="mt-4 grid items-start gap-4">
            ...
            <div className="space-y-4">
              <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                {!!programmePerformance.length && <div className="overflow-x-auto">
                  <div className="min-w-[760px]">
                    ...
                    <div className="divide-y divide-slate-100">{programmePerformance.map((row, index) => <div key={row.programme} className="grid grid-cols-[minmax(190px,1.5fr)_repeat(4,minmax(105px,0.8fr))_minmax(180px,1.25fr)] items-center gap-4 px-5 py-4 transition-colors hover:bg-[#f8fcf9]">
                      <div className="flex items-center gap-3"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${index % 3 === 2 ? "bg-[#fff4d9] text-[#d18a00]" : index % 3 === 1 ? "bg-[#eef5ff] text-[#3772ad]" : "bg-[#eaf8ef] text-[#0c8a49]"}`}><Building2 className="h-[18px] w-[18px]" /></div><div><strong className="text-sm text-[#173b2a]">{row.programme}</strong><p className="mt-0.5 text-[10px] text-slate-500">REA programme portfolio</p></div></div>
                      ...
                      <strong className="text-sm text-[#08733f]">{row.verified}%</strong>
                      <div className="flex items-center gap-3"><div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-[#42b976] to-[#08733f] transition-[width] duration-700 ease-out" style={{ width: `${row.verified}%` }} /></div><span className="w-9 text-right text-[11px] font-bold text-slate-600">{row.verified}%</span></div>
                    </div>)}</div>
                  </div>
                </div>}
              ...
              <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                <div className="mt-4 flex flex-wrap gap-4 text-[10px] text-slate-500">{[["#cbd5e1", "Submitted"], ["#5bc18d", "Verified"], ["#08733f", "Verification rate"]].map(([color, label]) => <span key={label} className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-sm" style={{ backgroundColor: color }} />{label}</span>)}</div>
                <div className="mt-3 h-[280px] sm:h-[320px]" aria-label="Animated project and verification trend chart"><ResponsiveContainer width="100%" height="100%"><ComposedChart key={JSON.stringify(filters)} data={trendData} margin={{ top: 16, right: 8, left: -18, bottom: 4 }}><CartesianGrid strokeDasharray="3 3" stroke="#e5ece7" vertical={false} /><XAxis dataKey="month" tick={{ fontSize: 11, fill: "#64748b" }} axisLine={false} tickLine={false} /><YAxis yAxisId="reports" allowDecimals={false} tick={{ fontSize: 10, fill: "#64748b" }} axisLine={false} tickLine={false} /><YAxis yAxisId="rate" orientation="right" domain={[0, 100]} tickFormatter={(value) => `${value}%`} tick={{ fontSize: 10, fill: "#64748b" }} axisLine={false} tickLine={false} /><Tooltip cursor={{ fill: "#f3f8f4" }} contentStyle={{ borderRadius: 10, borderColor: "#dbe7de", boxShadow: "0 10px 25px rgba(23,59,42,.10)", fontSize: 12 }} /><Bar yAxisId="reports" dataKey="submitted" name="Submitted" fill="#cbd5e1" radius={[4, 4, 0, 0]} isAnimationActive animationBegin={100} animationDuration={900} animationEasing="ease-out" /><Bar yAxisId="reports" dataKey="verified" name="Verified" fill="#5bc18d" radius={[4, 4, 0, 0]} isAnimationActive animationBegin={250} animationDuration={1000} animationEasing="ease-out" /><Line yAxisId="rate" type="monotone" dataKey="verificationRate" name="Verification rate" stroke="#08733f" strokeWidth={3} dot={{ r: 3.5, fill: "#08733f", stroke: "#ffffff", strokeWidth: 2 }} activeDot={{ r: 6 }} isAnimationActive animationBegin={450} animationDuration={1200} animationEasing="ease-in-out" /></ComposedChart></ResponsiveContainer></div>
              </section>
            </div>
          </div>
