...
const programmeStyles = [
  { icon: "bg-[#e8f1ff] text-[#286fc0]", bar: "from-[#75b5ff] to-[#286fc0]", accent: "text-[#286fc0]", hover: "hover:bg-[#f7faff]" },
  { icon: "bg-[#fff0e5] text-[#d46b20]", bar: "from-[#ffae73] to-[#d46b20]", accent: "text-[#c85f17]", hover: "hover:bg-[#fffaf6]" },
  { icon: "bg-[#e7f8ed] text-[#08733f]", bar: "from-[#55c583] to-[#08733f]", accent: "text-[#08733f]", hover: "hover:bg-[#f7fcf9]" },
  { icon: "bg-[#f0eafd] text-[#7452bd]", bar: "from-[#ad8bea] to-[#7452bd]", accent: "text-[#7452bd]", hover: "hover:bg-[#faf8ff]" },
  { icon: "bg-[#fff5d9] text-[#c58300]", bar: "from-[#f4c852] to-[#c58300]", accent: "text-[#b87900]", hover: "hover:bg-[#fffdf7]" },
];
...
export default function Index() {
  ...
  const programmePerformance = useMemo(() => {
    const portfolioRows = [...new Set(visibleProjects.map((project) => project.programme))].map((programme) => {
  ...
  const navContent = (
    <>
      <div className="veritas-rail-brand flex h-[94px] items-center gap-3 px-4"><AtlasMark /><div><p className="text-xl font-bold tracking-tight text-[#153b28]">REA</p><p className="mt-0.5 text-[7px] font-bold leading-[9px] text-[#173b2a]">RURAL ELECTRIFICATION<br />AGENCY</p></div></div>
      <div className="veritas-rail-separator h-px bg-slate-200" />
      <nav className="veritas-rail-nav flex-1 space-y-2 px-3 py-5">{visibleNavigation.map(({ label, icon: Icon }) => <button key={label} data-label={label} aria-label={label} onClick={() => { setActiveNav(label); setMobileMenuOpen(false); }} className={`veritas-rail-link flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-left text-sm font-medium transition-colors ${resolvedActiveNav === label ? "is-active bg-[#edf9f0] text-[#08733f]" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}><Icon className="h-[18px] w-[18px]" strokeWidth={resolvedActiveNav === label ? 2.5 : 1.8} /><span>{label}</span></button>)}</nav>
      <div className="veritas-rail-footer border-t border-slate-200 p-3"><button data-label="Settings" aria-label="Settings" className="flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50"><Settings className="h-[18px] w-[18px]" /><span>Settings</span></button></div>
    </>
  );
      ...
      <main className="lg:pl-[72px]">
        ...
        <div className="mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-7">
          {!resolvedActiveNav ? (
            ...
            <>
          <section className="rounded-b-xl border border-t-0 border-[#d6e9da] bg-[#f7fcf8] p-4"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 xl:items-end">{(Object.keys(filterDefaults) as FilterKey[]).map((key) => <div key={key} className="min-w-0"><FilterSelect label={filterLabels[key]} value={filters[key]} options={getFilterOptions(filters, key)} onChange={(value) => updateFilterWithDependencies(key, value)} /></div>)}<div className="flex flex-col gap-2 sm:col-span-2 lg:col-span-1 xl:col-span-1"><button onClick={() => setActiveNav("Claims")} className="flex h-10 items-center justify-center gap-2 whitespace-nowrap rounded-md bg-[#08733f] px-3 text-[11px] font-bold text-white hover:bg-[#065d32]"><ClipboardCheck className="h-4 w-4" /> Review Pending Reports ({visibleProjects.filter((project) => !project.verified).length})</button><button onClick={() => { setFilters(defaultFilters); setSelectedState("Kano"); resetMapView(); }} className="flex h-10 items-center justify-center gap-2 whitespace-nowrap rounded-md border border-[#76bd91] bg-white px-4 text-xs font-bold text-[#08733f] hover:bg-[#edf9f0]"><LocateFixed className="h-4 w-4" /> Reset filters</button></div></div></section>
          ...
          <div className="mt-4 grid items-start gap-4">
            <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-200 bg-gradient-to-r from-[#f8fcf9] to-white px-5 py-4"><h2 className="text-sm font-bold text-[#173b2a]">National Project Coverage</h2><p className="mt-1 text-xs text-slate-500">Explore national delivery by project count, installed capacity, or households reached</p><div className="mt-3 flex max-w-full gap-2 overflow-x-auto pb-1" aria-label="Map viewing mode">{mapModeOptions.map((option) => { const activeStyle = option.value === "projects" ? "border-[#08733f] bg-[#08733f] text-white shadow-[0_7px_16px_rgba(8,115,63,.18)]" : option.value === "capacity" ? "border-[#3772ad] bg-[#3772ad] text-white shadow-[0_7px_16px_rgba(55,114,173,.18)]" : "border-[#d28a00] bg-[#d28a00] text-white shadow-[0_7px_16px_rgba(210,138,0,.18)]"; const idleStyle = option.value === "projects" ? "border-[#cbe6d4] bg-[#f2faf5] text-[#08733f] hover:border-[#87c99e]" : option.value === "capacity" ? "border-[#cfdef1] bg-[#f2f7fd] text-[#3772ad] hover:border-[#94b8dc]" : "border-[#f0ddad] bg-[#fff9e9] text-[#b97800] hover:border-[#dfbd69]"; const Icon = option.value === "projects" ? FolderKanban : option.value === "capacity" ? Zap : Home; return <button key={option.value} type="button" onClick={() => setMapMode(option.value)} className={`group flex min-w-fit items-center gap-2 rounded-lg border px-3.5 py-2.5 text-[10px] font-bold transition-all duration-200 hover:-translate-y-0.5 ${mapMode === option.value ? activeStyle : idleStyle}`}><span className={`flex h-6 w-6 items-center justify-center rounded-md ${mapMode === option.value ? "bg-white/15" : "bg-white"}`}><Icon className="h-3.5 w-3.5" /></span>{option.label}</button>; })}</div></div>
            ...
            <div className="space-y-4">
              <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-col gap-2 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                  <div><h2 className="text-sm font-bold text-[#173b2a]">Programme Performance</h2><p className="mt-1 text-xs text-slate-500">Complete delivery and verification performance for the filtered portfolio</p></div>
                  <span className="text-[11px] font-semibold text-[#08733f]">{programmePerformance.length} active {programmePerformance.length === 1 ? "programme" : "programmes"}</span>
                </div>
                {!!programmePerformance.length && <div className="overflow-x-auto">
                  <div className="min-w-[760px]">
                    <div className="grid grid-cols-[minmax(190px,1.5fr)_repeat(4,minmax(105px,0.8fr))_minmax(180px,1.25fr)] gap-4 bg-slate-50 px-5 py-3 text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">
                      <span>Programme</span><span>Projects</span><span>Capacity</span><span>Households</span><span>Verified</span><span>Performance</span>
                    </div>
                    <div className="divide-y divide-slate-100">{programmePerformance.map((row, index) => { const style = programmeStyles[index % programmeStyles.length]; return <div key={row.programme} className={`grid grid-cols-[minmax(190px,1.5fr)_repeat(4,minmax(105px,0.8fr))_minmax(180px,1.25fr)] items-center gap-4 px-5 py-4 transition-all duration-200 ${style.hover}`}>
                      <div className="flex items-center gap-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl shadow-sm ${style.icon}`}><SolarCapacityIcon className="h-5 w-5" /></div><div className="min-w-0"><strong className="text-sm text-[#173b2a]">{row.programme}</strong><p className="mt-0.5 truncate text-[10px] text-slate-500">{row.fullName}</p></div></div>
                      <strong className="text-sm text-[#173b2a]">{row.projects.toLocaleString()}</strong>
                      <strong className="text-sm text-[#173b2a]">{row.capacity.toFixed(1)} MW</strong>
                      <strong className="text-sm text-[#173b2a]">{row.households.toLocaleString()}</strong>
                      <strong className={`text-sm ${style.accent}`}>{row.verified}%</strong>
                      <div className="flex items-center gap-3"><div className="h-2.5 flex-1 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full bg-gradient-to-r transition-[width] duration-1000 ease-out ${style.bar}`} style={{ width: `${row.verified}%` }} /></div><span className={`w-9 text-right text-[11px] font-bold ${style.accent}`}>{row.verified}%</span></div>
                    </div>; })}</div>
                  </div>
                </div>}
          ...
          <section className="mt-4 grid gap-4 lg:grid-cols-2">
            <article className="flex min-h-[330px] flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-start justify-between border-b border-slate-100 bg-gradient-to-r from-[#f5fbf7] to-white px-5 py-4"><div className="flex items-start gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#08733f] text-white shadow-[0_8px_18px_rgba(8,115,63,.18)]"><Clock3 className="h-5 w-5" /></div><div><h2 className="text-sm font-bold text-[#173b2a]">Recent Activity</h2><p className="mt-1 text-xs text-slate-500">Latest updates across filtered projects</p></div></div><button className="flex items-center gap-1 text-[11px] font-bold text-[#08733f] transition-colors hover:text-[#055c32]">View all<ArrowRight className="h-3.5 w-3.5" /></button></div>
              <div className="flex-1 px-5 py-2">{visibleProjects.slice(0, 4).map((project, index) => <div key={project.name} className="group relative flex items-center gap-3 border-b border-slate-100 py-3.5 last:border-0"><div className="relative flex shrink-0 flex-col items-center"><div className={`flex h-10 w-10 items-center justify-center rounded-xl ring-4 ring-white transition-transform group-hover:scale-105 ${project.verified ? "bg-[#e8f8ee] text-[#08733f]" : project.status === "Pending" ? "bg-[#fff4d9] text-[#c27a00]" : "bg-[#eaf3ff] text-[#3772ad]"}`}>{project.verified ? <CheckCircle2 className="h-5 w-5" /> : project.status === "Pending" ? <Clock3 className="h-5 w-5" /> : <FileCheck2 className="h-5 w-5" />}</div></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-xs font-bold text-[#173b2a]">{project.name}</p><span className={`h-1.5 w-1.5 shrink-0 rounded-full ${project.verified ? "bg-[#18a15b]" : project.status === "Pending" ? "bg-[#e6ad21]" : "bg-[#377fd2]"}`} /></div><p className="mt-1 text-[11px] text-slate-500">{project.verified ? "Inspection verified successfully" : project.status === "Pending" ? "Report awaiting review" : "Inspection update submitted"}</p></div><span className="shrink-0 rounded-full bg-slate-50 px-2.5 py-1 text-[10px] font-semibold text-slate-500">{index === 0 ? "Just now" : `${index + 1}d ago`}</span></div>)}</div>
            </article>
            <article className="flex min-h-[330px] flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-start gap-3 border-b border-slate-100 bg-gradient-to-r from-[#f3f7ff] to-white px-5 py-4"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#3772ad] text-white shadow-[0_8px_18px_rgba(55,114,173,.18)]"><Zap className="h-5 w-5" /></div><div><h2 className="text-sm font-bold text-[#173b2a]">Quick Actions</h2><p className="mt-1 text-xs text-slate-500">Common project and inspection tasks</p></div></div>
              <div className="grid flex-1 grid-cols-2 gap-3 p-4">
                <button onClick={() => setActiveNav("Projects")} className="group flex min-h-[112px] flex-col items-start justify-between rounded-xl border border-[#cce8d5] bg-gradient-to-br from-[#f1fbf4] to-white p-4 text-left transition-all hover:-translate-y-0.5 hover:border-[#86c99d] hover:shadow-md"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#daf4e3] text-[#08733f]"><FolderKanban className="h-5 w-5" /></span><span className="flex w-full items-end justify-between gap-2"><span><strong className="block text-xs text-[#173b2a]">Add Project</strong><small className="mt-1 block text-[10px] text-slate-500">Create a new record</small></span><ArrowRight className="h-4 w-4 text-[#08733f] transition-transform group-hover:translate-x-1" /></span></button>
                <button className="group flex min-h-[112px] flex-col items-start justify-between rounded-xl border border-[#cfdef2] bg-gradient-to-br from-[#f0f6fd] to-white p-4 text-left transition-all hover:-translate-y-0.5 hover:border-[#91b7df] hover:shadow-md"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#dfeeff] text-[#3772ad]"><Download className="h-5 w-5" /></span><span className="flex w-full items-end justify-between gap-2"><span><strong className="block text-xs text-[#173b2a]">Upload Report</strong><small className="mt-1 block text-[10px] text-slate-500">Import inspection data</small></span><ArrowRight className="h-4 w-4 text-[#3772ad] transition-transform group-hover:translate-x-1" /></span></button>
                <button onClick={() => setActiveNav("Claims")} className="group flex min-h-[112px] flex-col items-start justify-between rounded-xl border border-[#dcd9f3] bg-gradient-to-br from-[#f6f4fd] to-white p-4 text-left transition-all hover:-translate-y-0.5 hover:border-[#aaa2df] hover:shadow-md"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#ece9fb] text-[#6757bd]"><UsersRound className="h-5 w-5" /></span><span className="flex w-full items-end justify-between gap-2"><span><strong className="block text-xs text-[#173b2a]">Assign Inspector</strong><small className="mt-1 block text-[10px] text-slate-500">Allocate field work</small></span><ArrowRight className="h-4 w-4 text-[#6757bd] transition-transform group-hover:translate-x-1" /></span></button>
                <button onClick={() => setActiveNav("Claims")} className="group flex min-h-[112px] flex-col items-start justify-between rounded-xl border border-[#f1dca9] bg-gradient-to-br from-[#fff8e8] to-white p-4 text-left transition-all hover:-translate-y-0.5 hover:border-[#dfbd68] hover:shadow-md"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#fff0c9] text-[#c27a00]"><Clock3 className="h-5 w-5" /></span><span className="flex w-full items-end justify-between gap-2"><span><strong className="block text-xs text-[#173b2a]">Pending Review</strong><small className="mt-1 block text-[10px] text-slate-500">{visibleProjects.filter((project) => !project.verified).length} reports waiting</small></span><ArrowRight className="h-4 w-4 text-[#c27a00] transition-transform group-hover:translate-x-1" /></span></button>
              </div>
            </article>
          </section>
