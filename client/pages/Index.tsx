    ...
    <div className="min-h-screen bg-[#f6f8f6] text-slate-900">
      ...
      <main className="lg:pl-[72px]">
        ...
        <div className="mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-7">
          ...
          <section className="mt-3 flex gap-3 overflow-x-auto pb-1">{metrics.map(({ label, value, detail, icon: Icon, action }) => { const palette = label === "Installed Capacity" ? { card: "border-[#b8e0c6] bg-gradient-to-br from-[#eaf8ef] via-[#f4fbf6] to-white", icon: "bg-[#08733f] text-white", value: "text-[#075c33]", glow: "shadow-[0_8px_24px_rgba(8,115,63,.10)]", progress: "from-[#4bc27c] to-[#08733f]" } : label === "Projects" ? { card: "border-[#cbdcf2] bg-gradient-to-br from-[#edf5ff] via-[#f7faff] to-white", icon: "bg-[#3772ad] text-white", value: "text-[#285f96]", glow: "shadow-[0_8px_24px_rgba(55,114,173,.10)]", progress: "from-[#75b5ff] to-[#3772ad]" } : label === "Households Reached" ? { card: "border-[#ddd5f2] bg-gradient-to-br from-[#f3effc] via-[#faf8ff] to-white", icon: "bg-[#7452bd] text-white", value: "text-[#6344a9]", glow: "shadow-[0_8px_24px_rgba(116,82,189,.10)]", progress: "from-[#ad8bea] to-[#7452bd]" } : label === "Verification Rate" ? { card: "border-[#bfe5df] bg-gradient-to-br from-[#eaf9f7] via-[#f5fcfb] to-white", icon: "bg-[#168f83] text-white", value: "text-[#12776e]", glow: "shadow-[0_8px_24px_rgba(22,143,131,.10)]", progress: "from-[#54cabc] to-[#168f83]" } : { card: "border-[#efd9a3] bg-gradient-to-br from-[#fff5da] via-[#fffbef] to-white", icon: "bg-[#d28a00] text-white", value: "text-[#ad7100]", glow: "shadow-[0_8px_24px_rgba(210,138,0,.10)]", progress: "from-[#f2c04d] to-[#d28a00]" }; const cardClassName = `group relative min-h-[112px] min-w-[210px] flex-1 overflow-hidden rounded-xl border p-3.5 text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${palette.card} ${palette.glow}`; const cardContent = <><span className="absolute -right-5 -top-7 h-20 w-20 rounded-full bg-white/35 transition-transform duration-500 group-hover:scale-125" /><div className="relative flex h-full items-start gap-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl shadow-sm transition-transform duration-300 group-hover:rotate-3 group-hover:scale-105 ${palette.icon}`}><Icon className="h-5 w-5" /></div><div className="min-w-0 flex-1"><p className="text-xs font-bold text-[#344b3f]">{label}</p><p className={`mt-1.5 text-[22px] font-bold leading-none tracking-tight ${palette.value}`}>{value}</p><p className="mt-2 text-[10px] leading-4 text-slate-500">{detail}</p>{label === "Verification Rate" && <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/80"><div className={`h-full rounded-full bg-gradient-to-r transition-[width] duration-1000 ease-out ${palette.progress}`} style={{ width: value }} /></div>}</div></div></>; return action ? <button key={label} type="button" onClick={() => setActiveNav(action)} className={cardClassName} aria-label={`${label}: ${value}. Open verification queue`}>{cardContent}</button> : <article key={label} className={cardClassName}>{cardContent}</article>; })}</section>
