...
export default function Index() {
  ...
  const navContent = (
    <>
      ...
      <nav className="veritas-rail-nav flex-1 space-y-2 px-3 py-5" aria-label="REA navigation">{visibleNavigation.map(({ label, icon: Icon }) => <div key={label}>{label === "Overview" && <div className="px-3 pb-1 pt-1 text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Overview</div>}{label === "Field Inspections" && <div className="px-3 pb-1 pt-4 text-[9px] font-bold uppercase tracking-[0.14em] text-[#08733f]">M&amp;E Field Inspections</div>}{label === "Verification" && <div className="px-3 pb-1 pt-4 text-[9px] font-bold uppercase tracking-[0.14em] text-slate-500">Consultant Verification</div>}{label === "Reports" && <div className="px-3 pb-1 pt-4 text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Reporting &amp; Administration</div>}<button data-label={label} aria-label={label} onClick={() => { setActiveNav(label); setMobileMenuOpen(false); }} className={`veritas-rail-link flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-left text-sm font-medium transition-colors ${resolvedActiveNav === label ? "is-active bg-[#edf9f0] text-[#08733f]" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}><Icon className="h-[18px] w-[18px]" strokeWidth={resolvedActiveNav === label ? 2.5 : 1.8} /><span>{label}</span></button></div>)}</nav>
        ...
        <div className="mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-7">
          ...
          <div className="mt-4 grid items-start gap-4">
            <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-200 bg-gradient-to-r from-[#f8fcf9] to-white px-5 py-3"><h2 className="text-sm font-bold text-[#173b2a]">National Project Coverage</h2><p className="mt-0.5 text-[11px] text-slate-500">Explore national delivery by project count, installed capacity, or households reached</p><div className="mt-2 flex max-w-full gap-2 overflow-x-auto" aria-label="Map viewing mode">{mapModeOptions.map((option) => { const activeStyle = option.value === "projects" ? "border-[#08733f] bg-[#08733f] text-white shadow-sm" : option.value === "capacity" ? "border-[#3772ad] bg-[#3772ad] text-white shadow-sm" : "border-[#d28a00] bg-[#d28a00] text-white shadow-sm"; const idleStyle = option.value === "projects" ? "border-[#cbe6d4] bg-[#f2faf5] text-[#08733f] hover:border-[#87c99e]" : option.value === "capacity" ? "border-[#cfdef1] bg-[#f2f7fd] text-[#3772ad] hover:border-[#94b8dc]" : "border-[#f0ddad] bg-[#fff9e9] text-[#b97800] hover:border-[#dfbd69]"; const Icon = option.value === "projects" ? FolderKanban : option.value === "capacity" ? Zap : Home; return <button key={option.value} type="button" onClick={() => setMapMode(option.value)} className={`flex min-w-fit items-center gap-1.5 rounded-md border px-3 py-1.5 text-[10px] font-bold ${mapMode === option.value ? activeStyle : idleStyle}`}><span className={`flex h-5 w-5 items-center justify-center rounded ${mapMode === option.value ? "bg-white/15" : "bg-white"}`}><Icon className="h-3 w-3" /></span>{option.label}</button>; })}</div></div>
