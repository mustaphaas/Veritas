...
export default function Index() {
  ...
  const navContent = (
    <>
      ...
      <nav className="veritas-rail-nav flex-1 space-y-2 px-3 py-5">{visibleNavigation.map(({ label, icon: Icon }) => <button key={label} title={label} aria-label={label} onClick={() => { setActiveNav(label); setMobileMenuOpen(false); }} className={`veritas-rail-link flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-left text-sm font-medium transition-colors ${resolvedActiveNav === label ? "is-active bg-[#edf9f0] text-[#08733f]" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}><Icon className="h-[18px] w-[18px]" strokeWidth={resolvedActiveNav === label ? 2.5 : 1.8} /><span>{label}</span></button>)}</nav>
      <div className="veritas-rail-footer border-t border-slate-200 p-3"><button title="Settings" aria-label="Settings" className="flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50"><Settings className="h-[18px] w-[18px]" /><span>Settings</span></button></div>
    </>
  );
