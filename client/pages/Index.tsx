    ...
    <div className="min-h-screen bg-[#f6f8f6] text-slate-900">
      ...
      <main className="lg:pl-[76px]">
        ...
        <div className="mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-7">
          ...
          <section className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">{metrics.map(({ label, value, detail, icon: Icon, tone, action }) => { const cardClassName = `flex min-h-[128px] flex-col rounded-xl border p-4 text-left shadow-sm transition-colors ${tone === "highlighted" ? "border-[#cdebd6] bg-[#f4fbf6]" : tone === "pending" ? "border-[#f1dfaf] bg-[#fffaf0] hover:border-[#d9aa37] hover:bg-[#fff7df]" : "border-slate-200 bg-white"}`; const cardContent = <div className="flex h-full flex-col gap-3"><div className="flex items-center gap-3"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${tone === "pending" ? "bg-[#fff3cf] text-[#e29a00]" : "bg-[#e9f7ed] text-[#159455]"}`}><Icon className="h-5 w-5" /></div><p className="min-w-0 flex-1 text-sm font-semibold leading-tight text-[#263c31]">{label}</p></div><div className="mt-auto"><p className={`text-[22px] font-bold leading-none tracking-tight ${tone === "pending" ? "text-[#9a6300]" : "text-[#13281e]"}`}>{value}</p><p className="mt-2 text-[11px] leading-4 text-slate-500">{detail}</p>{label === "Verification Rate" && <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#119653]" style={{ width: value }} /></div>}</div></div>; return action ? <button key={label} type="button" onClick={() => setActiveNav(action)} className={cardClassName} aria-label={`${label}: ${value}. Open verification queue`}>{cardContent}</button> : <article key={label} className={cardClassName}>{cardContent}</article>; })}</section>
