...
const mapPalette = ["#effdf4", "#d2f8df", "#a6edbd", "#62d889", "#12864b"];
      ...
      <main className="lg:pl-[76px]">
        ...
        <div className="mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-7">
          ...
          <div className="mt-3 grid items-start gap-3">
            <section className="rea-national-map-card overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
              <div className="rea-national-map-header border-b border-slate-200 px-3 py-3"><div><p className="rea-map-eyebrow">National intelligence</p><h2 className="text-sm font-bold text-[#173b2a]">National Project Coverage</h2><small>Live state-level delivery footprint</small></div><div className="mt-3 inline-flex max-w-full overflow-x-auto rounded-md border border-slate-200 bg-white p-0.5" aria-label="Map viewing mode">{mapModeOptions.map((option) => <button key={option.value} type="button" onClick={() => setMapMode(option.value)} className={`whitespace-nowrap rounded px-3 py-2 text-[10px] font-semibold transition-colors ${mapMode === option.value ? "bg-[#08733f] text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"}`}>{option.label}</button>)}</div></div>
              <div className={selectedSummary ? "grid lg:grid-cols-[minmax(0,1fr)_290px]" : "grid"}>
                <div ref={mapContainerRef} className="rea-national-map-shell relative min-h-[390px] overflow-hidden bg-[#f7fbf8] sm:h-[390px]" onWheel={(event) => { event.preventDefault(); setMapZoom((zoom) => Math.min(2.5, Math.max(1, zoom + (event.deltaY < 0 ? 0.15 : -0.15)))); }}>
