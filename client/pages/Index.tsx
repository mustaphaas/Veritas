    ...
    <div className="min-h-screen bg-[#f6f8f6] text-slate-900">
      ...
      <main className="lg:pl-[72px]">
        ...
        <div className="mx-auto max-w-[1580px] px-4 py-0 sm:px-7 lg:px-7">
          ...
          <section className="mt-3 flex gap-3 overflow-x-auto pb-1">{metrics.map(({ label, value, detail, icon: Icon, action }) => { const palette = label === "Installed Capacity" ? { card: "border-[#b8e0c6] bg-[#eaf8ef]", icon: "bg-[#08733f] text-white", value: "text-[#075c33]", progress: "bg-[#08733f]" } : label === "Projects" ? { card: "border-[#cbdcf2] bg-[#edf5ff]", icon: "bg-[#3772ad] text-white", value: "text-[#285f96]", progress: "bg-[#3772ad]" } : label === "Households Reached" ? { card: "border-[#ddd5f2] bg-[#f3effc]", icon: "bg-[#7452bd] text-white", value: "text-[#6344a9]", progress: "bg-[#7452bd]" } : label === "Verification Rate" ? { card: "border-[#bfe5df] bg-[#eaf9f7]", icon: "bg-[#168f83] text-white", value: "text-[#12776e]", progress: "bg-[#168f83]" } : { card: "border-[#efd9a3] bg-[#fff5da]", icon: "bg-[#d28a00] text-white", value: "text-[#ad7100]", progress: "bg-[#d28a00]" }; const cardClassName = `min-h-[112px] min-w-[210px] flex-1 rounded-xl border p-3.5 text-left shadow-sm ${palette.card}`; const cardContent = <div className="flex h-full items-start gap-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl shadow-sm ${palette.icon}`}><Icon className="h-5 w-5" /></div><div className="min-w-0 flex-1"><p className="text-xs font-bold text-[#344b3f]">{label}</p><p className={`mt-1.5 text-[22px] font-bold leading-none tracking-tight ${palette.value}`}>{value}</p><p className="mt-2 text-[10px] leading-4 text-slate-500">{detail}</p>{label === "Verification Rate" && <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/80"><div className={`h-full rounded-full ${palette.progress}`} style={{ width: value }} /></div>}</div></div>; return action ? <button key={label} type="button" onClick={() => setActiveNav(action)} className={cardClassName} aria-label={`${label}: ${value}. Open verification queue`}>{cardContent}</button> : <article key={label} className={cardClassName}>{cardContent}</article>; })}</section>
