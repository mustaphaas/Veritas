...
export default function ReaFieldInspections() {
  ...
  const saveSectionAssignments = async () => {
    if (!selected || !token || !isTeamLead) return;
    setInspections((current) => current.map((item) => item.id === selected.id ? { ...item, sectionAssignments: draftSectionAssignments } : item));
    await api(`/api/field/rea-inspections/${selected.id}`, token, { method: "PATCH", body: JSON.stringify({ sectionAssignments: draftSectionAssignments }) });
    setSectionAssignModal(false);
    setMessage("Sections assigned");
    await load();
  };
