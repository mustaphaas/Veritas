            ...
            {!selected ? (
              <div className="overflow-x-auto">
                {inspections.length === 0 ? (
                  <div className="p-12 text-center"><ShieldCheck className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-600">No inspection assignments yet</p><p className="mt-1 text-xs text-slate-400">Create a team and assign an REA project.</p></div>
                ) : (
                  <div className="min-w-[760px]">
                    ...
                    {inspections.map((inspection) => {
                      ...
                      const filled = sections.filter((section) => section.fields.some((field) => inspection.form?.[field]?.trim())).length;
                      const progress = Math.round((filled / sections.length) * 100);
                      const status = inspection.status === "Verified" ? "Verified" : inspection.status === "Approved" ? "Approved" : inspection.status === "Submitted" ? "Submitted" : progress === 100 ? "Completed" : progress > 0 ? "In Progress" : "Not Started";
                      return <button key={inspection.id} type="button" onClick={() => { setSelectedInspection(inspection.id); setSelectedTeamId(inspection.teamId); setSelectedSection("project"); setShowSectionList(true); }} className="grid w-full grid-cols-[1.1fr_2fr_0.8fr_1.2fr_1fr] gap-4 border-b border-slate-100 px-5 py-4 text-left transition hover:bg-[#f7fcf8]">
                        <span className="truncate text-xs font-bold text-[#173b2a]">{team?.name || "—"}</span>
                        <span className="truncate text-xs text-slate-600">{project ? project.name : inspection.projectId}</span>
