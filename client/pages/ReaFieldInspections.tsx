            ...
            {!selected ? <div className="p-12 text-center"><ShieldCheck className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-600">No inspection assigned yet</p><p className="mt-1 text-xs text-slate-400">Create a team and assign an REA project.</p></div> : (
              <>
                ...
                <div className="grid md:grid-cols-[240px_minmax(0,1fr)]">
                  <div className="border-b border-slate-100 p-3 md:border-b-0 md:border-r">{sections.map((section) => { const done = section.fields.some((field) => selected.form?.[field]?.trim()); const assigned = assignedSections[section.id]; const assignee = selectedTeam?.members.find((member) => member.id === assigned); return <button key={section.id} onClick={() => setSelectedSection(section.id)} className={`mb-1 w-full rounded-lg p-3 text-left ${selectedSection === section.id ? "bg-[#edf9f0]" : "hover:bg-slate-50"}`}><div className="flex items-center gap-2"><span className={`flex h-6 w-6 items-center justify-center rounded-full ${done ? "bg-[#08733f] text-white" : "bg-slate-100 text-slate-400"}`}>{done ? <Check className="h-3.5 w-3.5" /> : <span className="text-[10px] font-bold">{sections.indexOf(section)+1}</span>}</span><span className="min-w-0 flex-1 truncate text-xs font-bold text-[#173b2a]">{section.title}</span><ChevronRight className="h-3.5 w-3.5 text-slate-300" /></div><div className="mt-1 pl-8 text-[9px] text-slate-400">{assignee ? `Assigned to ${assignee.name}` : "Unassigned"}</div></button>; })}</div>
                  ...
                  <div className="p-5">
                    {(() => { const section = sections.find((item) => item.id === selectedSection) || sections[0]; const assigned = assignedSections[section.id]; const assignee = selectedTeam?.members.find((member) => member.id === assigned); const canEdit = !selected.status || !["Submitted","Approved","Verified"].includes(selected.status); return <div>
                      <div className="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between"><div><h4 className="text-base font-bold text-[#173b2a]">{section.title}</h4><p className="mt-1 text-xs text-slate-500">{section.description}</p></div>{isTeamLead && <select value={assigned || ""} onChange={(event) => void updateAssignment(section.id, event.target.value)} className="h-9 rounded-md border border-slate-200 bg-white px-2 text-xs font-semibold text-[#173b2a]"><option value="">Assign section</option>{selectedTeam?.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select>}</div>
