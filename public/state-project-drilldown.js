(() => {
  ...
  const projects = Object.entries(STATE_TARGETS).flatMap(
  ...
  style.textContent = `
    ...
    svg[aria-label^="Interactive Nigeria state map"] path[data-drilldown-selected="true"] {
      stroke: #075c33 !important;
      stroke-width: 2.2px !important;
      filter: brightness(.96);
    }
    .veritas-state-project-drawer {
      position: absolute;
      z-index: 45;
      top: 12px;
      right: 12px;
      ...
      box-shadow: 0 20px 50px rgba(20,55,37,.20);
      backdrop-filter: blur(10px);
      font-family: "Montserrat", Inter, ui-sans-serif, system-ui, sans-serif;
    }
    .veritas-state-project-drawer * { box-sizing: border-box; }
    .veritas-state-project-head {
      padding: 16px 16px 13px;
      border-bottom: 1px solid #e5ebe7;
      background: linear-gradient(180deg,#fbfdfb 0%,#f5faf7 100%);
    }
    .veritas-state-project-title-row {
    ...
    .veritas-state-project-close {
      width:30px; height:30px; display:flex; align-items:center; justify-content:center;
      border:1px solid #dfe6e1; border-radius:8px; background:white; color:#617168; cursor:pointer;
    }
    ...
    .veritas-state-project-search input {
      width:100%; height:34px; border:1px solid #dce4de; border-radius:8px; background:#fafcfb;
      padding:0 10px; outline:none; font:500 10px/1 "Montserrat",Inter,sans-serif; color:#253a2f;
    }
    ...
    @media (max-width: 700px) {
      .veritas-state-project-drawer { left:12px; width:auto; }
    }
  `;
  ...
  function clearSelectedStateHighlight(svg) {
    svg?.querySelectorAll('path[role="button"]').forEach((path) => {
      path.removeAttribute("data-drilldown-selected");
    });
  }
  ...
  function renderRows(container, stateProjects, query = "") {
    ...
    const rows = needle
      ? stateProjects.filter((project) =>
          [project.name, project.programme, project.component, project.contractor, project.status]
            .join(" ")
            .toLowerCase()
            .includes(needle),
        )
    ...
    container.innerHTML = rows
      .map((project) => {
        ...
        return `
          <article class="veritas-state-project-row">
  ...
  function openStateProjects(state, clickedPath, svg) {
    ...
    clickedPath.setAttribute("data-drilldown-selected", "true");
    ...
    drawer.innerHTML = `
      <div class="veritas-state-project-head">
        <div class="veritas-state-project-title-row">
          <div>
            <h3 class="veritas-state-project-title">${escapeHtml(state)} State Projects</h3>
            <p class="veritas-state-project-subtitle">${stateProjects.length.toLocaleString()} projects in the current filtered view</p>
          </div>
          <button class="veritas-state-project-close" type="button" aria-label="Close ${escapeHtml(state)} projects">×</button>
        </div>
      ...
      <div class="veritas-state-project-search">
        <input type="search" placeholder="Search projects, programme or contractor" aria-label="Search ${escapeHtml(state)} projects" />
      </div>
      <div class="veritas-state-project-list"></div>
      <div class="veritas-state-project-foot">Click another state to switch project view · The national map stays fixed</div>
    `;
  ...
  // Remove the old default Kano details pane so the national map starts full-width.
  const observer = new MutationObserver(() => {
    const svg = document.querySelector('svg[aria-label^="Interactive Nigeria state map"]');
    if (svg) hideLegacyStatePanel(svg);
  });
