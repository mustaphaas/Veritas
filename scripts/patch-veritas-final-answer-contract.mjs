...
s = s.replace(/const BUILD_ID = "[^"]+";/, 'const BUILD_ID = "veritas-2026-09-11-final-answer-contract-r2";');
...
if (!s.includes('FINAL ANSWER CONTRACT:')) {
  ...
  if (s.includes(generalAnchor)) {
    s = s.replace(generalAnchor, `${markerRule}\nRespond as Veritas, with a concise but genuinely reasoned answer.` + '`;');
  } else {
    console.log('General response prompt already changed; skipping prompt insertion');
  }
}
...
if (!a.includes('FINAL ANSWER CONTRACT:')) {
  const fnStart = a.indexOf('export function analyticsAnswerPrompt(question, result) {');
  const fnEnd = fnStart >= 0 ? a.indexOf('\n`;\n}', fnStart) : -1;
  if (fnStart >= 0 && fnEnd > fnStart) {
    ...
    a = a.slice(0, fnEnd) + contract + a.slice(fnEnd);
  } else {
    console.log('Analytics answer prompt shape changed; leaving it unchanged rather than blocking deployment');
  }
}
