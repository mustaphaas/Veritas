...
w = w.replace(/const BUILD_ID = "[^"]+";/, 'const BUILD_ID = "veritas-2026-09-11-professional-opening-r1";');
...
if (!a.includes('professional management readout that sounds like an experienced REA officer')) {
  if (!a.includes(analyticsAnchor)) throw new Error('analytics response anchor not found');
  const afterOld = '2. Follow with "What the data confirms" and give only the few figures needed to support that readout. Do not dump the full result set unless the user explicitly asks for all rows.\n3. Follow with "What this may mean" only when interpretation is useful. Explain the management risk or implication without presenting a possible cause as a fact.\n4. Follow with "What management should review next" and give practical checks or actions tied directly to the confirmed finding.\n5. End with a short bottom line only when it adds something new.\n';
  a = a.replace(analyticsAnchor + afterOld, analyticsBlock);
}
