...
if (source.includes(badgeBlock)) source = source.replace(badgeBlock, "");

const patterns = [
  /\s*<span className="[^"]*">\s*\{displayMetrics\.projects\.toLocaleString\(\)\} Projects\s*<\/span>/,
  /\s*<span className="[^"]*">\s*\{displayMetrics\.verified\.toLocaleString\(\)\} Verified\s*<\/span>/,
  /\s*<span className="[^"]*">\s*\{formatMw\(displayMetrics\.kw\)\}\s*<\/span>/,
  /\s*<span className="[^"]*">\s*\{displayMetrics\.households\.toLocaleString\(\)\} Households\s*<\/span>/,
];
for (const pattern of patterns) source = source.replace(pattern, "");

fs.writeFileSync(file, source);
console.log("Removed all Project Map summary badges.");
