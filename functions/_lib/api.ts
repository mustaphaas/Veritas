import { ApiError, bodyJson, fail, json, methodNotAllowed, numberValue, routeParts, sameOrigin, text } from "./http";
import { clearSessionCookie, cookieValue, hashPassword, randomToken, sessionCookie, sha256, verifyPassword } from "./security";
...
// Minimal shapes for the real Cloudflare D1Database / R2Bucket runtime
// objects, covering only the methods this file uses. Defined by hand
// (rather than via @cloudflare/workers-types, which isn't a project
// dependency) so generic calls like .first<T>() still type-check.
...
export async function dispatch(request: Request, env: Bindings): Promise<Response> {
